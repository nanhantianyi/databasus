...
func testTestConnectionBackupPrivilegesFromActiveRole(
	...
	fixture.executeStatements(t,
		...
		fmt.Sprintf("SET DEFAULT ROLE ALL TO '%s'@'%%'", fixture.Username),
		"FLUSH PRIVILEGES",
	)
...
// mysqldump logs in fresh, and with activate_all_roles_on_login off a granted-but-inactive role
// gives it nothing, so reading only the active roles is what matches what the dump will have.
// The direct INSERT grant is what lets the account reach the schema at all and puts the table in
// the dump set; everything it would need to read that table sits in the unactivated role.
